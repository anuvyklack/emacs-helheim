...
(defcustom ibuffer-vc-skip-remote-files t
  "If non-nil, don't query the VC status of remote files."
  :type 'boolean
  :group 'ibuffer-vc)

(defcustom ibuffer-vc-ignore-buffer-p #'ibuffer-vc--default-ignore-buffer-p
  "Predicate which tells whether a given BUFFER should be exclued from grouping.
Should be a function that takes BUFFER object and return non-nill if it should
...
(defun ibuffer-vc-read-filter ()
  "Read a cons cell of the form: (backend-name . root-dir)."
  (cons (car (-> (completing-read "VC backend: " vc-handled-backends nil t)
                 (read-from-string)))
...
(defun ibuffer-vc-root (buffer)
  "Return a cons cell (BACKEND . ROOT-DIR) if given BUFFER corresponds to the
file that is under version control. Otherwise return nil.

- BACKEND is a symbol that is a name of the vc backend.
- ROOT-DIR is a string with the path to the vc root."
  (if-let* (((not (funcall ibuffer-vc-ignore-buffer-p buffer)))
            (filename (funcall ibuffer-vc-buffer-file-name-function buffer))
            ((ibuffer-vc--include-file-p filename))
            (backend (vc-responsible-backend filename :no-error))
            ;; (backend (ibuffer-vc--deduce-backend filename))
            )
      (let* ((root-fn-name (intern (format "vc-%s-root" (downcase (symbol-name backend)))))
             (root-dir (if (fboundp root-fn-name) ;; git, svn, hg, bzr (at least)
                           (funcall root-fn-name filename)
                         (pcase backend
                           ((and (or 'darcs 'DARCS)
                                 ;; `vc-darcs' is an external package
                                 (guard (fboundp 'vc-darcs-find-root)))
                            (vc-darcs-find-root filename))
                           ((or 'cvs 'CVS) (vc-find-root filename "CVS"))
                           ((or 'rcs 'RCS) (or (vc-find-root filename "RCS")
                                               (concat filename ",v")))
                           ((or 'sccs 'SCCS) (or (vc-find-root filename "SCCS")
                                                 (concat "s." filename)))
                           ((or 'src 'SRC) (or (vc-find-root filename ".src")
                                               (concat filename ",v")))
                           (_ (error "ibuffer-vc: unknown vc backend '%s'" backend))))))
        (cons backend root-dir))))

(defun ibuffer-vc--default-ignore-buffer-p (buffer)
  "Exclude help buffers from grouping."
  (memq (buffer-local-value 'major-mode buffer)
        ibuffer-help-buffer-modes))

(defun ibuffer-vc-buffer-file-truename (buffer)
  "Return the truename of the file associated with BUFFER.

The file associated with BUFFER is the one that BUFFER is visiting,
whose file name is stored in the buffer-local variable
`buffer-file-name'.

If that is nil, but BUFFER sets the variable
...
If neither of those is set for BUFFER, return nil."
  (or (-some-> (or (buffer-base-buffer buffer)
                   buffer)
        (buffer-file-name)
        (file-truename))
      (buffer-local-value 'list-buffers-directory buffer)))

(defun ibuffer-vc--skip-buffer-p (buffer))

(defun ibuffer-vc--include-file-p (file)
  "Return t if FILE should be included in ibuffer-vc's filtering."
  (and file
       (or (null ibuffer-vc-skip-remote-files)
...
;;       (vc-responsible-backend file :no-error)
;;     (or (vc-backend file)
;;         (seq-filter (lambda (b) (vc-call-backend b 'responsible-p file))
;;                     vc-handled-backends))))
...
(defun ibuffer-vc--status-string ()
  ...
  (when (and buffer-file-name
             (ibuffer-vc--include-file-p buffer-file-name))
    (-if-let (state (ibuffer-vc--state buffer-file-name))
        (symbol-name state)
      "-")))
...
(define-ibuffer-column vc-relative-file
  ( :name "Filename")
  (if buffer-file-name
      ;; (if-let* ((root (cdr (ibuffer-vc-root buffer))))
      ;;     (file-relative-name buffer-file-name root)
      ;;   (abbreviate-file-name buffer-file-name))
      (-if-let ((_ . root) (ibuffer-vc-root buffer))
          (file-relative-name buffer-file-name root)
        (abbreviate-file-name buffer-file-name))
    ""))
...
(define-ibuffer-column vc-status-mini
  ...
  (if (and buffer-file-name (ibuffer-vc--include-file-p buffer-file-name))
      (pcase (ibuffer-vc--state buffer-file-name)
        ('added "A")
        ('removed "D")
        ('up-to-date "=")
        ('edited "*")
        ('needs-update "O")
        ((or 'conflict 'needs-merge 'unlocked-changes) "!")
        ('ignored "I")
        ((or 'nil 'unregistered 'missing) "?"))
    " "))
